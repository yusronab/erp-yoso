...
interface FormulaProduksiItem {
    formulaUtamaId: number;
}

const ModalItem = ({ data, formulaId }: { data: any[], formulaId: number }) => {
    ...
    const handleClickFormula = (id: number) => {
        setLoading(true);

        axios.post('/api/sub-item', {
            ...
            <div className="tooltip" data-tip="Tahapan produksi">
                ...
                <div className={showModal ? 'modal modal-open' : 'modal'}>
                    <div className="modal-box w-11/12 max-w-5xl">
                        <p className="font-bold mb-3">List Formula Item</p>
                        <hr />
                        ...
                        <table className="table bordered">
                            ...
                            <tbody>
                                {filteredData.slice(startIndex, endIndex).map((row, index) => {
                                    const checkData = row.subItem.some(
                                        (item: FormulaProduksiItem) => item.formulaUtamaId === formulaId
                                    );
                                    return (
